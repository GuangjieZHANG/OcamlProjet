...
(* This function aimes to print the graph to fomat graphviz*)
let export path graph =
 ...
 (* out node [shape = circle];*)
(* Vertices *)
  v_iter graph (fun vi -> out " %s " vi.id) ;
  out ";\n";
 
(* Edges *)
  v_iter graph (fun vi -> List.iter (fun (label, id2) -> out "     LR_%s -> LR_%s  [ label = %s ];\n" vi.id id2 label) vi.outedges ) ;
  out "}\n" ;
