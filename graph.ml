...
(* This function is to convert string lables to int lables *)
let map graph vmap emap = 
 (* This function is to change every edge in vetex_info *)
  let rec aux = function 
    |[]->[]
    |(e,id)::rest->((emap e),id)::(aux rest)
    in 
 (* This function aimes to change every valeur in hashtable , returns a new vetex_info*)
  let change info =
  {
   label = vmap info.label;
   id=info.id;
   outedges=(aux info.outedges);
   inedges=(aux info.inedges)
  }
   in
 (* Create a new hashtbl to change every vertext info , using iterator *)
 (* For the first time I write 'resultat' but he wants a graph in fact *)
   let resultat = map_hashtbl graph.vertices change in
     {
       vertices=resultat
      };;
